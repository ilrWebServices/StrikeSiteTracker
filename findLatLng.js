...
module.exports = async (content) => {
  ...
  while (index < contentArray.length) {
    ...
    if (element.Address && !element["Latitude, Longitude"]) {
      const address = `${element.Address.trim()}, ${element["Zip Code"]}, ${
        element.City
      }, ${element.State}`;
      const addressURI = address.replace(/ /g, "+");
      // console.log(addressURI)
      const result = await axios.get(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${addressURI}&key=${GEOCODING_API}`
      );
      const latlngObj = _get(result, "data.results[0].geometry.location", null);
      // console.log(latlngObj)
      element["latlngObj"] = convertLatLngObjectToString(latlngObj);
    }

function Multiselect(a) {
    ...
    "undefined" != typeof $ ? this._item = a[0] : this._item = a,
    this._createUI(),
    this._appendEvents(),
...
if (!m_helper)
    var m_helper = {
        ...
        textField: function(a) {
            return a || (a = new Object),
            a.tag = "input",
            a.attributes || (a.attributes = new Object),
...
Multiselect.prototype = {
    ...
    _createItemList: function() {
        var a = m_helper.create({
          ...
          , b = this;
        m_helper.each(this._getItems(this._item), function(c) {
            var d = b._createItem("li", c.id, c.text, c.selected);
            a.appendChild(d);
            var e = d.querySelector("input[type=checkbox]");
            c.multiselectElement = e,
            e.dataset.multiselectElement = JSON.stringify(c)
        });
    ...
    _createItem: function(a, b, c, d) {
        var e = m_helper.checkbox({
          ...
          , h = m_helper.label();
        return f.className = "multiselect-text",
        f.innerHTML = c,
        h.appendChild(e),
        h.appendChild(f),
        g.appendChild(h),
    ...
    _initSelectedFields: function() {
        var a = this._getItems().filter(function(a) {
            return a.selected
        });
        if (0 != a.length) {
            var b = this;
            m_helper.each(a, function(a) {
                b.select(a.id)
            })
        } else
            this.selectAll();
    ...
    select: function(a) {
        var b = this;
        a && m_helper.each(document.getElementById(this._getIdentifier()).querySelectorAll(".multiselect-checkbox"), function(c) {
            c.dataset.val == a && (m_helper.check(c),
            b._onCheckBoxChange(c, b))
        })
    },
    ...
    _appendEvents: function() {
        var a = this;
        document.getElementById(a._getInputFieldIdentifier()).addEventListener("focus", function(b) {
            a._showList(a),
            document.getElementById(a._getInputFieldIdentifier()).value = "",
            m_helper.each(window.multiselects, function(c) {
                document.getElementById(c._getItemListIdentifier()).offsetParent && m_helper.parent(b.target, c._getIdentifier()) && c._hideList(a)
            })
        }),
        ...
        document.getElementById(a._getIdentifier()).addEventListener("click", function(b) {
            b = b || window.event;
            var c = b.target || b.srcElement;
            m_helper.parent(c, a._getIdentifier()) && b.stopPropagation()
        }),
        document.getElementById(a._getItemListIdentifier()).addEventListener("mouseover", function() {
            a._showList(a)
        }),
        m_helper.each(document.getElementById(a._getIdentifier()).querySelectorAll(".multiselect-checkbox"), function(b) {
            b.addEventListener("change", function(c) {
                a._onCheckBoxChange(b, a, c)
            })
        });
        var b = function() {
            ...
            if (b && "" != b) {
                ...
                var c = Array.prototype.filter.call(document.querySelectorAll("#" + a._getItemListIdentifier() + " li span"), function(a) {
                    return a.innerHTML.toLowerCase().indexOf(b) > -1
                });
                m_helper.hideAll(document.querySelectorAll("#" + a._getItemListIdentifier() + " li")),
                m_helper.each(c, function(a) {
                    m_helper.show(a.parentElement.parentElement)
                })
            } else
    ...
    _onCheckBoxChange: function(a, b, c) {
    ...
    _performSelectAll: function(a, b) {
        ...
        m_helper.each(c, function(a) {
            m_helper.setActive(a.multiselectElement.parentElement.parentElement),
            m_helper.select(b._item.options[a.index]),
            m_helper.check(a.multiselectElement)
        })) : (b._itemCounter = 0,
        m_helper.each(c, function(a) {
            a.multiselectElement.parentElement.parentElement.classList.remove("active"),
            b._item.options[a.index].removeAttribute("selected"),
            m_helper.uncheck(a.multiselectElement)
        }))
    },
    ...
    _forceUpdate: function() {
        ...
        0 != this._itemCounter) {
            ...
            this._itemCounter < 10 ? (a.style.left = "-45px",
            b.style.marginLeft = "-42px") : this._itemCounter < 100 ? (a.style.left = "-50px",
            b.style.marginLeft = "-47px") : this._itemCounter < 1e3 ? (a.style.left = "-55px",
            b.style.marginLeft = "-52px") : this._itemCounter < 1e4 && (a.style.left = "-60px",
    ...
    _getItems: function() {
        if (void 0 == this._items) {
            for (var a = [], b = this._item.options, c = 0; c < b.length; c++) {
                var d = {
                    id: b[c].value,
                    index: c,
                    text: b[c].innerHTML,
...
window.multiselects = [],
"undefined" != typeof $ ? ($.fn.multiselect = function() {
    0 != this.length && $(this).each(function(a, b) {
        window.multiselects.push(new Multiselect($(b)))
    })
}
,
$(document).click(function(a) {
    hideMultiselects(a)
})) : (document.multiselect = function(a) {
    m_helper.each(document.querySelectorAll(a), function(a) {
        window.multiselects.push(new Multiselect(a))
    })
}
,
